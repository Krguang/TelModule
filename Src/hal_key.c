...
void keyXingShortPress(void)
{
	printf("key xing press\n");
	//sendOneBit(0x0b);
	keyFlag[10] = !keyFlag[10];
}

void keyJingShortPress(void)
{
	printf("key jing press\n");
	//sendOneBit(0x0c);
	keyFlag[11] = !keyFlag[11];
}

void keyMianTiShortPress(void)
{

	printf("key mianti press\n");
	if (HAL_GPIO_ReadPin(TEL_HFI_GPIO_Port,TEL_HFI_Pin))
	{
		HAL_GPIO_WritePin(TEL_HFI_GPIO_Port, TEL_HFI_Pin, GPIO_PIN_RESET);
	}
	else 
	{
		HAL_GPIO_WritePin(TEL_HFI_GPIO_Port, TEL_HFI_Pin, GPIO_PIN_SET);
	}
}
...
void keyInit(void)
{
	singleKey[0] = keyInitOne(KEY0_GPIO_Port, KEY0_Pin, key0ShortPress, NULL);
	singleKey[1] = keyInitOne(KEY1_GPIO_Port, KEY1_Pin, key1ShortPress, NULL);
	singleKey[2] = keyInitOne(KEY2_GPIO_Port, KEY2_Pin, key2ShortPress, NULL);
	singleKey[3] = keyInitOne(KEY3_GPIO_Port, KEY3_Pin, key3ShortPress, NULL);
	singleKey[4] = keyInitOne(KEY4_GPIO_Port, KEY4_Pin, key4ShortPress, NULL);
	singleKey[5] = keyInitOne(KEY5_GPIO_Port, KEY5_Pin, key5ShortPress, NULL);
	singleKey[6] = keyInitOne(KEY6_GPIO_Port, KEY6_Pin, key6ShortPress, NULL);
	singleKey[7] = keyInitOne(KEY7_GPIO_Port, KEY7_Pin, key7ShortPress, NULL);
	singleKey[8] = keyInitOne(KEY8_GPIO_Port, KEY8_Pin, key8ShortPress, NULL);
	singleKey[9] = keyInitOne(KEY9_GPIO_Port, KEY9_Pin, key9ShortPress, NULL);
	singleKey[10] = keyInitOne(KEY_XING_GPIO_Port, KEY_XING_Pin, keyXingShortPress, NULL);
	singleKey[11] = keyInitOne(KEY_JING_GPIO_Port, KEY_JING_Pin, keyJingShortPress, NULL);
	singleKey[12] = keyInitOne(KEY_MianTi_GPIO_Port, KEY_MianTi_Pin, keyMianTiShortPress, NULL);
